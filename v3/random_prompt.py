...
pose_variations = [
    "Sitting", "Standing", "Lying", "Jumping", "Climbing", "Rope", "Kneeling", "Chair pose", "With stick", "With sword", "With big sword",
    "With pistols", "Underwater", "Ballet", "Casual", "A-pose", "T-pose", "With a spear", "With a gun", "Duo", "Lifting something heavy",
    "Contraposto", "Pointing", "Punching", "Kicking", "Walking", "Running"
]
...
categories = [
    ...
    "Automobiles", "Military", "Aviation", "Weapons", "Cloth", "Buildings", "Landscapes", "Facial expressions", "Common Objects"
]
...
def random_direction():
    azimuth = round_to_nearest(random.uniform(0, 360))
    elevation = round_to_nearest(random.uniform(0, 360))
...
def truncated_normal(a, b, mean=None, std_dev=None):
    ...
    while True:
        # random.gauss is built-in and much faster to load
...
def generate_palette(harmony="random", num_colors=3,
    ...
    for idx, hue in enumerate(hues):
        # optionally, you can differentiate the first color.
        if idx == 0:
            # make the first color a bit more neutral.
...
def print_palette():
    """
    Generate and print a palette using ANSI escape codes.
    """
...
def complete_pose_prompt():
    print_bar_with_title(" POSE PROMPT ")
    print_random_perspective()
    print(f" > Sex: {random_sex()}")
    print(f" > Physique: {random_physique()}")
    print(f" > Pose: {random_pose()}")
    ...
    print_palette()
    print_random_restriction(prob=0.06)
    time_limit = print_random_paper_size_and_time_limit([2, 94, 4], 3, 80)
    print_last_bar()
    return time_limit
...
def random_complete_prompt():
    complete_prompt_functions = [
        complete_daily_exercise_prompt,
        complete_face_prompt, complete_face_part_prompt,
        complete_pose_prompt, complete_anatomy_prompt, 
        complete_specific_anatomy_prompt, complete_anatomy_motion_prompt, 
...
def complete_daily_plan():
    """
    Exercise: "normal exercise" | "daily exercise"
    Human body: invent pose | study anatomy | study model | hands / feet focus
    Face: whole face | face part
    Other studies: 
    """
...
def test_1():
    print(random_physique())
    print(random_sex())
    print(random_size())
    print(random_anatomy(4))
    print(random_anatomy_motion())
    print(random_face_part())
    print(random_emotion_to_body_expression(4))
    print(random_traditional_mediums())
    print(random_exercise())
    print(random_texture())
    print(random_light_source())
    print(random_pose())
    print("----")
    print(random_lvl(random_anatomy))
    print(random_lvl(random_emotion_to_body_expression))


def test_2():
    complete_pose_prompt()
    complete_anatomy_prompt()
    complete_face_prompt()
    complete_exercise_prompt()
    complete_anatomy_motion_prompt()
    complete_face_part_prompt()
    complete_specific_anatomy_prompt()
    

def test_3():
    a = set()
    for _ in range(50):
        a.add(generate_palette().__str__())
    print(len(a))
...
def test_6():
    for _ in range(10):
        print(random_paper_size())
    print(random_restriction())


def test_7():
    for _ in range(50):
        print_random_restriction()


def test_8():
    for _ in range(10):
        print_random_paper_size_and_time_limit([10, 80, 5], 5, 60)
        print()
...
def test_9():
    ...
    print(f" > Paper size: {paper_size}; Mean: {round(mean)}; Standard deviation: {round(std_dev)}")
    total = 0
    max_iter = 10
    for _ in range(max_iter):
        val = random_time_limit(a, b, mean=mean, std_dev=std_dev)
        print(f" > Time limit: {val} min")
        total += val

    print(f"Média: {round(total / max_iter)}")
