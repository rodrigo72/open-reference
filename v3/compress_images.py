...
def should_compress(image_path,
                    size_threshold=500*1000,
                    width_threshold=1920,
                    height_threshold=1080,
                    max_pixels=89478485,  # same as Pillow's default
    ...
    try:
        with Image.open(image_path) as img:
            ...
            if total_pixels > max_pixels:
                if debug:
                    print(f"Image '{image_path}' exceeds max pixels ({total_pixels} > {max_pixels}). Deleting.")
                try:
                    ...
                    if debug:
                        print(f"Failed to delete '{image_path}': {remove_err}")
                return False
            if width < width_threshold and height < height_threshold:
                return False
    except Exception as e:
        if debug:
            print(f"Failed to open/process image '{image_path}': {e}. Deleting it.")
        try:
            os.remove(image_path)
            if debug:
                print(f"Deleted '{image_path}'.")
